...
def pulse_shaping_mode_from_mcstas_mode(mode: str) -> PulseShapingMode:
    """Return the BEER pulse-shaping mode matching a McStas mode value."""
    mode = str(mode)
    if (pulse_shaping_mode := MCSTAS_PULSE_SHAPING_MODES.get(mode)) is not None:
        return pulse_shaping_mode

    normalized = mode.upper()
    for pulse_shaping_mode in PulseShapingMode:
        if normalized == pulse_shaping_mode.value:
            return pulse_shaping_mode

    raise ValueError(f'Mode {mode} is not a known BEER pulse-shaping mode.')


def mcstas_source_position() -> Position[snx.NXsource, RunType]:
    """Return the source position used for analytical McStas frame unwrapping."""
    return sc.vector([0.0, 0.0, 0.0], unit='m')


def mcstas_pulse_shaping_choppers(
...
def mcstas_detector_ltotal(
    ...
    return da.coords['moderator_to_detector_distance']
