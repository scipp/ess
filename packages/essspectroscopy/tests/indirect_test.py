...
from scipp import vector, scalar, array, sqrt
...
def vectors_close(a, b, tol=None):
    from scipp import dot, sqrt, scalar
    if tol is None:
...
def test_back_scattering_sample_analyzer_vector():
    ...
    analyzer_transform = rotations_from_rotvecs(vector([0., 0., 0.], unit='degree'))
    ...
    detector_positions = analyzer_position + analyzer_orientation * analyzer_orientation * detector_positions
    calculated = secondary.sample_analyzer_vector(sample_position, analyzer_position, analyzer_orientation, detector_positions)
...
def test_sample_analyzer_vector():
    ...
    detector_positions = scalar(1.0, unit='m') * z + wires
    detector_positions = analyzer_position + analyzer_orientation * analyzer_orientation * detector_positions
    calculated = secondary.sample_analyzer_vector(sample_position, analyzer_position, analyzer_orientation,
                                                  detector_positions)
    assert all_vectors_close(calculated, sample_analyzer_vec + wires / 2.0)
