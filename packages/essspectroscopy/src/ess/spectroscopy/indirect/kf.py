...
def sample_analyzer_vector(
    ...
    y = vector([0, 1, 0], unit=analyzer_orientation.unit)  # and y perpendicular to the scattering plane
...
def fixed_tau_hat_sample_analyzer_vector(
    ...
    from numpy.linalg import solve as linalg_solve, matrix_rank, lstsq as linalg_least_sqr
    from scipp import any as sc_any, cross, concat, scalar, vector, dot, vectors
    from ess.spectroscopy.utils import norm

    # Scipp does not distinguish between coordinates and directions, so we need to do some extra legwork
    # to ensure we can apply the orientation transformation _and_ obtain a dimensionless direction vector
    o = vector([0, 0, 0], unit=analyzer_orientation.unit)
    x = vector([1, 0, 0], unit=analyzer_orientation.unit)  # McStas defines x parallel to Bragg plane tau
    y = vector([0, 1, 0], unit=analyzer_orientation.unit)  # and y perpendicular to the scattering plane
    # z is in the Bragg plane and in the scattering plane

    tau, yhat = [v / norm(v) for v in [(analyzer_orientation * ei - analyzer_orientation * o) for ei in (x, y)]]

    v_d = detector_position - sample_position
    n_v_d = norm(v_d)
    if sc_any(n_v_d == scalar(0., unit=sample_position.unit)):
        bad = n_v_d == scalar(0., unit=sample_position.unit)
    ...
    b_1 = dot(sample_position, n_1).to(unit=sample_position.unit)     # (N_sample, N_det) or (N_det,)
    b_2 = dot(analyzer_position, n_2).to(unit=sample_position.unit)   # (N_analyzer,)
    b_3 = dot(0.5 * sample_position + 0.5 * detector_position, n_3).to(unit=sample_position.unit)

    if n_1.ndim > 0 or n_2.ndim > 0 or n_3.ndim > 0 or b_1.ndim > 0 or b_2.ndim > 0 or b_3.ndim > 0:
    ...
    new_dim = str(uuid4())
    a = concat([n_1, n_2, n_3], dim=new_dim).transpose(dims=n_1.dims + (new_dim,)).values  # (..., 3, 3)
    b = concat([b_1, b_2, b_3], dim=new_dim).transpose(dims=n_1.dims + (new_dim,)).values  # (..., 3)
