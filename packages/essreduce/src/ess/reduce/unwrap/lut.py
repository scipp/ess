...
def _polygon_intersections(polygons, xs):
...
def _compute_mean_wavelength(
    ...
    ----------
    """
    # at_sample = frames.propagate_to(sc.linspace('distance', 55., 75., N, unit='m'))
    # subf = at_sample.frames[-1].subframes
...
def _compute_frame_sequence(
    ...
    pulse_frequency = 1.0 / pulse_period.to(unit='s')
    ...
    for dist in distances:
        ...
        if selected_frame is None:
            raise ValueError(
                ...
                f"has distance {frames[0].distance:c}."
            )

        # subframes = frames.propagate_to(dist).frames[-1].subframes
