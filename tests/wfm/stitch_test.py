...
def test_basic_stitching():
    ...
    frames['time_correction'] = sc.array(
        dims=['frame'], values=[shift], unit=sc.units.us
    )
    frames["wfm_chopper_mid_point"] = sc.vector(value=[0.0, 0.0, 2.0], unit='m')

    data = sc.DataArray(
        data=sc.ones(dims=['t'], shape=[100], unit=sc.units.counts),
        coords={
            't': sc.linspace(dim='t', start=0.0, stop=10.0, num=101, unit=sc.units.us),
    ...
    assert sc.identical(
        sc.values(stitched),
        sc.DataArray(
            data=sc.ones(dims=['tof'], shape=[nbins], unit=sc.units.counts) * nbins,
            coords={
                'tof': sc.linspace(
                    dim='tof',
                    start=0.0 - shift,
                ...
                'source_position': sc.vector(value=[0.0, 0.0, 2.0], unit='m'),
            },
        ),
    )
...
def _do_stitching_on_beamline(wavelengths, dim, event_mode=False):
    ...
    if event_mode:
        ...
        num = 2001
    time_binning = sc.linspace(
        dim=dim, start=(tmin - dt).value, stop=(tmax + dt).value, num=num, unit=dt.unit
    )
    events = sc.DataArray(
        data=sc.ones(
            dims=['event'],
...
def _check_lambda_inside_resolution(
    lam, dlam_over_lam, data, event_mode=False, check_value=True
):
...
def test_stitching_on_beamline(event_mode, dim):
    wavelengths = sc.array(
        dims=['event'], values=[1.75, 3.2, 4.5, 6.0, 7.0, 8.25], unit='angstrom'
    )
    ...
    for i in range(len(wavelengths)):
        _check_lambda_inside_resolution(
            wavelengths['event', i],
            dlambda_over_lambda,
            stitched,
...
def test_stitching_on_beamline_bad_wavelength(event_mode, dim):
    ...
    # the WFM choppers.
    wavelengths = sc.array(
        dims=['event'], values=[1.5, 3.2, 4.5, 6.0, 7.0, 8.25], unit='angstrom'
    )
    stitched, dlambda_over_lambda = _do_stitching_on_beamline(
        wavelengths, dim=dim, event_mode=event_mode
    )
