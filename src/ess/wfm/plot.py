...
def time_distance_diagram(data: sc.DataArray, **kwargs) -> plt.Figure:
    ...
    ax.add_patch(
        Rectangle(
            (0, 0),
            (
                2.0 * data.meta["source_pulse_t_0"] + data.meta["source_pulse_length"]
            ).value,
    ...
    ax.text(
        data.meta["source_pulse_t_0"].value,
        -pulse_rectangle_height,
        "Source pulse ({} {})".format(
            data.meta["source_pulse_length"].value,
    ...
    for name in ch.find_chopper_keys(data):
        ...
        ax.text(
            2.0 * time_close[-1] - time_open[-1], yframe, name, ha="left", va="bottom"
        )
    ...
    for i in range(frames.sizes["frame"]):
        ...
        lambda_max = np.array(
            [
                [data.meta["source_pulse_t_0"].value, 0],
                [(data.meta["source_pulse_t_0"] + frame['delta_time_max']).value, 0],
        ...
        ax.plot(
            np.concatenate((lambda_min[:, 0], lambda_min[0:1, 0])),
        ...
        ax.plot(
            np.concatenate((lambda_max[:, 0], lambda_max[0:1, 0])),
...
def frames_before_stitching(
    data: sc.DataArray, frames: sc.Dataset, dim: str, bins_per_frame: int = 32
):
    """
    ...
    for i in range(frames_no_shift['time_min'].sizes['frame']):
        key = 'frame{}'.format(i)
        out[key] = stitch(
            frames=frames_no_shift['frame', i : i + 1],
            data=summed,
            dim=dim,
            bins=bins_per_frame,
        )
...
def frames_after_stitching(
    ...
    for i in range(frames['time_min'].sizes['frame']):
        key = 'frame{}'.format(i)
        out[key] = stitch(
            frames=frames['frame', i : i + 1], data=summed, dim=dim, bins=bins_per_frame
        )
    return sc.plot(out)
